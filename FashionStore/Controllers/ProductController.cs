    ...
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> Search(string? value)
        {
            ...
            if (value != null)
            {
                ...
                var filteredProducts = products.Where(x => x.ProductName.ToUpper().Contains(searchTerm)).ToList();
        ...
        public async Task<IActionResult> Sorting(int? typesoft)
        {
            var categories = await _context.Categories.Include(x => x.Products).ToListAsync();
            var products = await _context.Products
                .Include(p => p.Brand)
            ...
            switch (typesoft)
            {
                case 1:
                    ...
                    break;
                case 2:
                    model.Products = model.Products.Where(p => p.Price >= 0 && p.Price <= 55).ToList();
                    break;
                case 3:
                    model.Products = model.Products.Where(p => p.Price > 55 && p.Price <= 100).ToList();
                    break;
                default:
                    break;
            }
